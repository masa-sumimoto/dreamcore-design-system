  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 flex items-center justify-between px-6 py-5 md:px-8 bg-background/40 backdrop-blur-md border-b border-surface-dim/15">
        <Link
          href="/"
          className="font-display text-xl transition-colors duration-300 hover:text-rose"
      ...
      <div
        className={`fixed inset-0 z-40 bg-background/95 backdrop-blur-lg flex flex-col items-center justify-center transition-all duration-500 ease-fade md:hidden ${
        ...
        <nav className="flex flex-col items-center gap-10 text-center">
          {links.map((link, i) => (
            <Link
              ...
              href={link.href}
              onClick={toggleMenu}
              className={`font-display text-4xl text-midnight/80 transition-all duration-700 ease-drift hover:text-rose ${
